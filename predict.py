...
class Predictor(BasePredictor):
    def setup(self):
        """Download and load FastConformer Quran NeMo model once during startup"""
        print("Downloading FastConformer Quran NeMo model from HuggingFace...")
        nemo_model_path = hf_hub_download(
            repo_id="NightPrince/stt-ar-fastconformer-quran-minshawi",
            filename="quran_minshawi_final.nemo"
        )
        print("Loading FastConformer ASR model...")
        self.model = nemo_asr.models.ASRModel.restore_from(
            restore_path=nemo_model_path,
            map_location="cpu"
        )
        self.model.eval()
        print("FastConformer Quran ASR model loaded successfully.")
    ...
    def predict(
        ...
        # 2. Transcribe audio with NeMo FastConformer
        ...
        # 3. Group words into video subtitle segments
        segments = []
        current_words = []
